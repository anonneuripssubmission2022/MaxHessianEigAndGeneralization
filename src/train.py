...
def main(dataset, arch_id, batch_size, lr, opt='sgd', loss='ce', network_seed=0, train_seed=1, rho=0.05, acc_goal=0.99, abridged_size=1000, physical_batch_size=DEFAULT_PHYS_BATCH_SIZE, disable_dropout=False, between_iterates=False, save_freq=10):
    path = get_path(dataset, lr, batch_size, arch_id, opt, loss, rho=rho)
    results_dir = os.environ["RESULTS"]
    directory = f"{results_dir}/{path}_{network_seed}"

    makedirs(directory, exist_ok=True)
    makedirs(f"{directory}/leading_eigenvalues", exist_ok=True)
    ...
    save_every = lambda epoch: (epoch%save_freq==0)
    abs_tol = 1.0 - acc_goal
    
    torch.manual_seed(train_seed)
    train_gd(directory, network, loss_fn, acc_fn, train_dataset, test_dataset, optimizer, batch_size, verbose=True, print_freq=1, physical_batch_size=physical_batch_size, should_save = save_every, sam=sam, abs_tol=abs_tol)
    ...
    if between_iterates:
        train_gd_extra_iter(directory, network, loss_fn, acc_fn, train_dataset, test_dataset, optimizer, batch_size, verbose=True, print_freq=1, physical_batch_size=physical_batch_size, should_save = save_every, sam=sam, abs_tol=abs_tol)
    ...
    torch.save(leading_eig, f"{directory}/leading_eigenvalues/leading_eigenvalue_final")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--acc_goal", type=float, help="terminate training if the train accuracy ever crosses this value", default=0.99)
    ...
    main(dataset=args.dataset, arch_id=args.arch_id, batch_size=args.batch_size, lr=args.lr, opt=args.opt, loss=args.loss, network_seed=args.network_seed, train_seed=args.train_seed, rho=args.rho, acc_goal=args.acc_goal, abridged_size=args.abridged_size, physical_batch_size=args.physical_batch_size, disable_dropout=args.disable_dropout, between_iterates=args.between_iterates, save_freq=args.save_freq)
